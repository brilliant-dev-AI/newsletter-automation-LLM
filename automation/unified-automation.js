...
class UnifiedAutomationService {
  ...
  async init() {
    ...
    try {
      ...
      console.log(
        "✅ Browser launched successfully with playwright-aws-lambda",
      );
    } catch (error) {
...
async function runSkyvernLogic(page, url, email) {
  ...
  try {
    ...
    if (!emailInput) {
      return {
        success: false,
        error: "AI could not identify email field",
        framework: "skyvern",
      };
    }
    ...
    let successMessage =
      "Form submitted successfully with Skyvern AI automation";
    let additionalSuccessFound = false;
...
async function runBrowserbaseLogic(page, url, email) {
  console.log(
    "☁️ Running Browserbase-style automation (cloud-like features)...",
  );
  ...
  try {
    ...
    if (!emailInput) {
      return {
        success: false,
        error: "Cloud browser could not identify email field",
        framework: "browserbase",
      };
    }
    ...
    let successMessage =
      "Form submitted successfully with Browserbase cloud automation";
    let additionalSuccessFound = false;
    ...
    for (const indicator of successIndicators) {
      try {
        ...
        if (element) {
          additionalSuccessFound = true;
          successMessage =
            "Successfully subscribed with Browserbase cloud automation";
          break;
        }
      } catch (e) {
        continue;
...
async function runAutomation(url, email, framework) {
  ...
  try {
    console.log(
      `🚀 Starting ${framework} automation with Puppeteer + Chrome Lambda...`,
    );
    ...
    try {
      ...
      console.log(
        "✅ Browser launched successfully with modern Puppeteer + @sparticuz/chromium",
      );
    } catch (browserError) {
      console.error("❌ Browser launch failed:", browserError.message);
      throw new Error(
        `Browser launch failed on Lambda: ${browserError.message}`,
      );
    }
      ...
      if (
        ...
        while (
          ...
          if (
            ...
            !newContent.toLowerCase().includes("challenge")
          ) {
          ...
          cloudflareAttempts++;
          console.log(
            `⏳ Still waiting for Cloudflare... (attempt ${cloudflareAttempts}/15)`,
          );
        }

        if (cloudflareAttempts >= 15) {
          console.log(
            `❌ Cloudflare challenge timeout, trying newsletter discovery anyway`,
          );
        }
      }
    ...
    if (!newsletterFound) {
      console.log(
        `🔍 Step 2: Searching header and footer only for newsletter/subscribe links`,
      );
      ...
      try {
        ...
        const newsletterLink = await page.evaluate(() => {
          ...
          const newsLink = document.querySelector(
            'a[href*="/newsletters"][href*="ref=header_nav"]',
          );
          const subscribeLink = document.querySelector(
            'a[data-test="header-nav-link-subscribe"]',
          );

          // Check for exact "News" text link (Product Hunt specific)
          const newsTextLink = Array.from(document.querySelectorAll("a")).find(
            (link) =>
          ...
          const subscribeTextLink = Array.from(
            document.querySelectorAll("a"),
          ).find(
            (link) =>
              link.textContent.trim() === "Subscribe" &&
              (link.href.includes("newsletters") ||
                link.href.includes("campaign=weekly_newsletter") ||
                link.href.includes("ref=header_nav")),
          );
        ...
        console.log(
          `🔗 Found ${allLinks.length} sample links on page:`,
          allLinks,
        );
        ...
        console.log(
          `🔘 Found ${allButtons.length} sample buttons on page:`,
          allButtons,
        );
        ...
        if (newsletterLink && newsletterLink.visible) {
          console.log(
            `🔗 Found newsletter link: "${newsletterLink.text}" -> ${newsletterLink.href}`,
          );

          try {
            const href = newsletterLink.href;
            console.log(
              `🔗 Following newsletter link from header/footer: ${href}`,
            );
            ...
            if (emailField) {
              ...
              console.log(
                `❌ No email field found at header/footer link: ${href}`,
              );
            }
          } catch (e) {
            console.log(
              `❌ Failed to follow header/footer newsletter link: ${e.message}`,
            );
          }
        } else {
          console.log(
            `❌ No newsletter/subscribe links found in header or footer`,
          );
        }
      } catch (e) {
    ...
    if (!newsletterFound) {
      return {
        ...
        message:
          "Newsletter discovery completed: checked given URL and searched header/footer for newsletter links, but no email signup forms were found",
      };
    }
...
if (require.main === module) {
  async function testUnifiedAutomation() {
    ...
    try {
      ...
      const testUrl =
        process.env.TEST_URL || "https://www.producthunt.com/newsletter";
