  ...
  async storeExtractedLinks(emailId, links) {
    ...
    try {
      ...
      const promises = links.map(async (link, index) => {
        const params = {
          TableName: process.env.LINKS_TABLE || 'NewsletterLinks',
          Item: {
            id: `${emailId}-${index}`,
            emailId: emailId,
            url: link.url,
...
${extractedLinks.length > 10 ? `... and ${extractedLinks.length - 10} more` : ''}
  ...
  isNewsletterEmail(email) {
    ...
    // For testing purposes, accept ALL emails (very lenient)
    console.log(`🔧 Testing mode: accepting all emails as newsletters`);
    return true;
  }
