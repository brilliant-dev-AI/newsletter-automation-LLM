...
class EmailService {
  constructor() {
    ...
    // Email configuration removed - we use Zapier for email forwarding
  }

  // Email generation removed - we use Zapier for email forwarding

  // Email forwarding setup removed - we use Zapier
  ...
  async processIncomingEmail(emailData) {
    ...
    try {
      ...
      if (!this.isNewsletterEmail(email)) {
        console.log(
          `⚠️ Email doesn't appear to be a newsletter, skipping processing`,
        );
        return {
          success: false,
      ...
      // Store the extracted links in DynamoDB
      console.log(
        `💾 About to store ${extractedLinks.length} links for email ${emailId}`,
      );
      await this.storeExtractedLinks(emailId, extractedLinks);
      console.log(
        `✅ Successfully stored ${extractedLinks.length} links for email ${emailId}`,
      );
  ...
  async storeEmail(email) {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      console.log(
        `📧 Email content: ${JSON.stringify(email, null, 2).substring(0, 200)}...`,
      );
      return key;
    }

    const bucketName = process.env.EMAIL_BUCKET || "divizend-newsletter-emails";
  ...
  async extractLinksFromEmail(email) {
    ...
    try {
      ...
      console.log(
        `✅ Extracted ${uniqueLinks.length} unique links (${llmLinks.length} via LLM)`,
      );

      // Send to n8n for Google Sheets integration (bonus feature)
      await this.sendToN8n(uniqueLinks, email);
  ...
  async extractLinksWithLLM(email, basicLinks) {
    ...
    try {
      ...
      const basicLinksText = basicLinks
...
Be thorough - newsletters often contain many valuable links that might be missed by basic parsing.`;
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert at extracting and categorizing links from newsletter emails. Always return valid JSON.",
            },
            {
              role: "user",
              content: prompt,
  ...
  async storeExtractedLinks(emailId, links) {
    ...
    console.log(
      `🔗 Attempting to store ${links.length} links for email: ${emailId}`,
    );
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      links.forEach((link, index) => {
        console.log(
          `🔗 Link ${index + 1}: ${link.url} (${link.category}, relevance: ${link.relevance})`,
        );
      });
      return;
    }
    ...
    try {
      ...
      const promises = links.map(async (link, index) => {
        ...
        console.log(
          `🔗 Storing link ${index + 1}: ${link.url} in table: ${params.TableName}`,
        );
  ...
  // Email sending functions removed - we use Zapier for email forwarding
  ...
  isNewsletterEmail(email) {
    ...
    console.log(
      `🔍 Newsletter detection: subjectMatch=${subjectMatch}, bodyMatch=${bodyMatch}, hasUnsubscribe=${hasUnsubscribe}, linkCount=${linkCount}, hasEmailContent=${hasEmailContent}`,
    );
    ...
    const isLikelyNewsletter =
      ...
      hasUnsubscribe ||
      hasMultipleLinks ||
      (hasAnyLinks && hasEmailContent);

    console.log(
      `🔍 Newsletter detection result: ${isLikelyNewsletter ? "ACCEPTED" : "REJECTED"}`,
    );
  ...
  async getAllExtractedLinks() {
    console.log(
      `🔍 Fetching links from table: ${process.env.LINKS_TABLE || "LinksTable"}`,
    );

    // Check if we're in local development mode
    if (process.env.NODE_ENV === "development") {
      console.log(
        "🔧 Running in development mode - returning empty array (no local DynamoDB)",
      );
      return [];
    }
    ...
    try {
      ...
      console.log(
        `📊 DynamoDB scan result: ${result.Items?.length || 0} items found`,
      );
      return result.Items || [];
    } catch (error) {
  ...
  /**
   * Send extracted links to n8n for Google Sheets integration
   */
  async sendToN8n(extractedLinks, emailInfo) {
    if (!process.env.N8N_WEBHOOK_URL) {
      console.log(
        "⚠️ n8n webhook URL not configured, skipping n8n integration",
      );
      return;
    }
    ...
    try {
      console.log(
        `🔗 Sending ${extractedLinks.length} links to n8n for Google Sheets integration`,
      );

      // Send each link as a separate row to Google Sheets via n8n
      for (const link of extractedLinks) {
        const payload = {
          newsletter_subject: emailInfo.subject || "Unknown Subject",
          newsletter_from: emailInfo.from || "Unknown Sender",
          newsletter_date: emailInfo.date || new Date().toISOString(),
          link_url: link.url,
          link_text: link.text || "",
          link_context: link.context || "",
          link_category: link.category || "other",
          link_relevance: link.relevance || 5,
          extraction_method: link.extractionMethod || "html_parser",
          extracted_at: link.extractedAt || new Date().toISOString(),
        ...
        console.log(`✅ Link sent to n8n: ${link.url.substring(0, 50)}...`);
      }

      console.log(
        `🎉 Successfully sent ${extractedLinks.length} links to n8n for Google Sheets export`,
      );
    } catch (error) {
      console.error("❌ n8n integration error:", error.message);
      // Don't throw - this is a bonus feature, shouldn't break main functionality
    }
  }
}
