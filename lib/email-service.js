...
class EmailService {
  ...
  async processIncomingEmail(emailData) {
    ...
    try {
      ...
      console.log(`💾 About to store ${extractedLinks.length} links for email ${emailId}`);
      await this.storeExtractedLinks(emailId, extractedLinks);
      console.log(`✅ Successfully stored ${extractedLinks.length} links for email ${emailId}`);
...
Be thorough - newsletters often contain many valuable links that might be missed by basic parsing.`;
  ...
  async getAllExtractedLinks() {
    ...
    // Check if we're in local development mode
    if (process.env.NODE_ENV === 'development') {
      console.log('🔧 Running in development mode - returning empty array (no local DynamoDB)');
      return [];
    }
  ...
  async sendToN8n(extractedLinks, emailInfo) {
    ...
    try {
      console.log(`🔗 Sending ${extractedLinks.length} links to n8n for Google Sheets integration`);
      ...
      console.log(`🎉 Successfully sent ${extractedLinks.length} links to n8n for Google Sheets export`);
