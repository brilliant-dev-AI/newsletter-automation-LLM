...
export function LinksDisplay({ className = "" }: LinksDisplayProps) {
  ...
  if (error) {
    return (
      <div
        className={`p-8 bg-red-50 border border-red-200 rounded-2xl ${className}`}
      >
  ...
  const llmExtractedCount = links.filter(
    (link) => link.extractionMethod === "llm",
  ).length;
    ...
    <div className={className}>
      ...
      {links.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-sm">
            Newsletter emails will appear here once processed
          </p>
        </div>
      ) : (
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {links.map((link) => (
            <div
              key={link.id}
              className="p-4 border rounded-lg bg-white border-gray-300"
            >
